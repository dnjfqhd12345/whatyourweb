...
<div id="{{ id }}" class="star-ratings" data-max-rating="{{ star_count }}" data-avg-rating="{{ rating.average }}">
    ...
    <div class="star-ratings-rating-average">{% trans '별점: ' %} <span class='star-ratings-rating-value'>{{ rating.average|floatformat:"-2" }}</span></div>
    {% endblock rating_average %}
    {% if a %}
    {% block rating_count %}
    <div class="star-ratings-rating-count">{% trans '평가 개수: ' %} <span class='star-ratings-rating-value'>{{ rating.count }}</span></div>
    {% endblock rating_count %}
    {% block rating_user %}
    <div class="star-ratings-rating-user">{% trans '당신의 평가: ' %} <span class='star-ratings-rating-value' data-when-null="{% trans '' %}">{% if user_rating %}{{ user_rating.score }}{% else %}{% trans '' %}{% endif %}</span></div>
